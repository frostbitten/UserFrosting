(function( $ ) {
    $.fn.flashAlerts = function() {
        ...
        .then(function( data ) {        // Pass the deferral back
            ...
            var alertHTML = "";
            if (data) {
                $.each(data, function(alert_idx, alert_message) {
                    if (alert_message['type'] == "success"){
                        alertHTML += "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>" + alert_message['message'] + "</div>";
                    } else if (alert_message['type'] == "warning"){
                        alertHTML += "<div class='alert alert-warning alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>" + alert_message['message'] + "</div>";
                    } else 	if (alert_message['type'] == "info"){
                        alertHTML += "<div class='alert alert-info alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>" + alert_message['message'] + "</div>";
                    } else if (alert_message['type'] == "danger"){
                        alertHTML += "<div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>" + alert_message['message'] + "</div>";
                    }
                });
            }
...
// Set jQuery.validate settings for bootstrap4 integration
var elStor;
$.validator.setDefaults({
    ...
    unhighlight: function(element) {
        if($(element).parent('.input-group').attr('data-toggle') =='tooltip') {
			$(element).parent('.input-group').attr('data-original-title','').tooltip('hide');
		}
    ...
    errorPlacement: function(error, element) {
		elStor = element;
        if(element.parent('.input-group').attr('data-toggle') =='tooltip') {
			element.parent('.input-group').attr('data-original-title',$(error).text()).tooltip('show');
		}else if(element.parent('.input-group').prevAll('label').length) {
            error.insertAfter(element.parent().prevAll('label'));
    ...
    success: function(element) {
        ...
        elStor.closest('.form-group').find('.form-control-feedback').remove();
        elStor.closest('.form-group').addClass('has-success has-feedback');
        elStor.after('<i class="fa fa-check form-control-feedback" aria-hidden="true"></i>');
    }
});
...
$(function(){
	...
	$('.sidebar-info').each(function(){
		stcount++;
		$(this).append('<div class="tether-sidebar-helper" id="sinfo-'+stcount+'"></div>');
	});
	...
	$('.sidebar-info').each(function(){
		stcount++;
		$target = $(this).parent();
		//$(this).appendTo('#sidebarfeed');
		sidebarTethers[stcount] = new Tether({
		  element: $(this),
		  target: $target,
		  ...
		  constraints:
			  [
				{
				  to: document.getElementById('sidebarfeed'),
				  pin: ['right']
				}, 
				{
				  to: $('.sidebar-wrapper')[0],
				  pin: ['top']
				},
				{
				  to: $('#sinfo-0')[0],
	...
	var i=0;
	var totoaldrop = $('.drop-container:not(.drop-ignore-on-init)').length;
	$('.drop-container:not(.drop-ignore-on-init)').each(function(){
		$container = $(this);
		$target = $container.find('.drop-target');
		extraclass = $container.data('class');
		content = $container.find('.drop-content').wrapInner('<div data-drop-i="'+i+'" class="'+extraclass+'"></div>').html();
		openOn = 'click';
		theme = $container.data('theme');
		dropPosition = 'bottom right'; //default
		
		//choose new dropPosition if this is a in sidebar tethered item
		dropPosition = $container.parents('.sidebar-info').length ? 'bottom left' : dropPosition;
		
		$target.addClass(theme);
		i++;
		finalclasses = theme;
		if($container.parents('.navbar-fixed-top').length){
			finalclasses += " navdrop";
		}
		drop[i] = new Drop({
		  target: $target[0],
		  content: content,
	...
	$('.fittext').each(function(){
		ratio = $(this).attr('data-ratio') === "undefined" ? 1 : $(this).attr('data-ratio');
		minsize = $(this).attr('data-minsize') === "undefined" ? "8px" : $(this).attr('data-minsize');
		maxsize = $(this).attr('data-maxsize') === "undefined" ? "200px" : $(this).attr('data-maxsize');
		$(this).fitText(ratio, {minFontSize: minsize, maxFontSize: maxsize});
	});

	$('.match-heights-row').each(function(){
		$thisrow = $(this);
		targets = $(this).attr("data-match-heights-targets").split(",");
		targets.forEach(function(t){
			$thisrow.find(t).matchHeight();
		});
	});
	
	$('.sidebar-wrapper a[href="'+document.location.origin+document.location.pathname+'"]').closest('.list-group-item').addClass('active');
	$('.sidebar-wrapper a[href="'+document.location.origin+document.location.pathname+'"]').parents('.collapse').each(function(){
		$('#'+$(this).attr('id')).collapse('show');
	});
	
});

$(window).on("load shown.bs.collapse hidden.bs.collapse", function() {
	$(window).trigger('resize scroll');
});
if($('#side-menu').length && $('.sidebar-feed').length){
...
function resizeFunc() {
	if($('#side-menu').height()-$('.sidebar-nav').height()>5){
			$('.sidebar-nav').css('overflow-y','auto');
		}else{
			$('.sidebar-nav').css('overflow-y','hidden');
		}
	sidebarmaxh = $(window).innerHeight()-$('.sidebar-feed').position().top-60;
	if(sidebarmaxh>60){
		$('.sidebar-feed').css('max-height',sidebarmaxh);
	}else{

}
	$('.sidebar-nav').css('max-height',$(window).innerHeight()-$('.sidebar-nav').position().top-$('.sidebar-wrapper').position().top);
}
...
function ufFormSubmit(formElement, validators, msgElement, successCallback, msgCallback, beforeSubmitCallback) {
    formElement.validate({
        ...
        submitHandler:  function (f, event) {
            ...
            if (!submit_button) {submit_button = form.find(".btn.submit");}
            if (submit_button) {
